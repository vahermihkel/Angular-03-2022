...
export class ShopsComponent implements OnInit, AfterViewInit  {
  ...
  // @Output() initMapEvent: EventEmitter<any> = new EventEmitter();
  // @Output() setMapViewEvent: EventEmitter<any> = new EventEmitter();
  ...
  onZoomShop(shopName: string) {
    ...
    if (shopFound) {
      // const shopLocation = {latLng: [shopFound.latitude, shopFound.longitude], zoom: 13};
      // this.setMapViewEvent.emit(shopLocation);
      this.mapComponent.map.setView(L.latLng([shopFound.latitude, shopFound.longitude]),13);
    } else {
      // const shopLocation = {latLng: [59.4341, 24.7489], zoom: 11};
      // this.setMapViewEvent.emit(shopLocation);
